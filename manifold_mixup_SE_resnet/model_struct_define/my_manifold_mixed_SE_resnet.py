...
from model_struct_define.squeeze_excitation import SELayer
...
class SEBasicBlock(nn.Module):
    expansion = 1  # can i use this variation!?!?!?!?!?!?!?!? #########################################################

    def __init__(self, inplanes, planes, stride=1, downsample=None, reduction=16, shake_shake=False, use_se=False):
        ...
        self.downsample = downsample  # what is 'downsample'? ##########################################################
        self.stride = stride
...
class SEBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, reduction=16, use_se=False, shake_shake=False):
...
class origin_ResNet(nn.Module):
    # This ResNet does Manifold-Mixup.
    # https://arxiv.org/pdf/1806.05236.pdf
    def __init__(self, block, layers, num_classes=2, zero_init_residual=True, mixup_hidden=False, use_se=False, shake_shake=False):
        super(origin_ResNet, self).__init__()
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        for _ in range(1, blocks):
            ...
            layers.append(block(self.inplanes, planes, shake_shake=self.shake_shake, use_se=self.use_se))
...
class my_ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, zero_init_residual=True, mixup_hidden=False, use_se=False, shake_shake=False):
        ...
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # why multiple 'widen_factor'? It's just 1.
        self.layer1 = self._make_layer(block, 64*widen_factor, layers[0])
        self.layer2 = self._make_layer(block, 128*widen_factor, layers[1], stride=2)
        self.layer3 = self._make_layer(block, 256*widen_factor, layers[2], stride=2)
        self.layer4 = self._make_layer(block, 512*widen_factor, layers[3], stride=2)
        ...
        # self.features = nn.Sequential(self.conv1, self.bn1, self.maxpool, self.layer1, self.layer2, self.layer3, self.layer4, self.avgpool)
    ...
    def forward(self, x, lam=None, target=None, flag_extract=False):
        def mixup_process(out, target_reweighted, lam):
            # target_reweighted is one-hot vector
            # target is the taerget class.
        ...
        out = self.layer3(out)
        out_feature = self.layer4(out)


        out_feature = self.avgpool(out_feature)
        out_feature = out_feature.view(out_feature.size(0), -1)
        out = self.fc(out_feature)
        out_fc = out
        if flag_extract:
            if lam is None:
                return out, [out_feature, out_fc]
            else:
                return out, target_reweighted, [out_feature, out_fc]
        else:
            if lam is None:
                return out
            else:
...
def se_resnet18(num_classes, if_se=False, if_mixup=False, if_shake_shake=False):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = my_ResNet(SEBasicBlock, [2, 2, 2, 2], num_classes=num_classes, use_se=if_se, mixup_hidden=if_mixup, shake_shake=if_shake_shake)
...
def se_resnet34(num_classes, if_mixup=False):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = my_ResNet(SEBasicBlock, [3, 4, 6, 3], num_classes=num_classes, mixup_hidden=if_mixup)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model
...
def se_resnet50(num_classes, if_mixup=False):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = my_ResNet(SEBottleneck, [3, 4, 6, 3], num_classes=num_classes, mixup_hidden=if_mixup)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model
...
def se_resnet101(num_classes, if_mixup=False):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = my_ResNet(SEBottleneck, [3, 4, 23, 3], num_classes=num_classes, mixup_hidden=if_mixup)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model
...
def se_resnet152(num_classes, if_mixup=False, if_shake_shake=False):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = my_ResNet(SEBottleneck, [3, 8, 36, 3], num_classes=num_classes, mixup_hidden=if_mixup, shake_shake=if_shake_shake)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model