...
parser.add_argument('--id', type=str, default='SE_resnet')
...
SE_resnet_model = se_resnet18(9, if_mixup=False, if_se=True, if_shake_shake=False)
...
optimizer = optim.Adam(SE_resnet_model.parameters(), lr=args.lr)

num_epochs = 1000
for epoch in range(num_epochs):
    SE_resnet_model.train()  # Set model to training mode
    running_loss = 0.0
    correct_train = 0
    total_train = 0
    for images, labels in train_loader:
        ...
        outputs = SE_resnet_model(images)  # Forward pass
    ...
    # Validation
    SE_resnet_model.eval()  # Set model to evaluation mode
    val_loss = 0.0
    correct_val = 0
    total_val = 0
    with torch.no_grad():
    ...
    #     save_model_path = os.path.join(save_model_dir, "SE_resnet_final_model.pt")
    #     torch.save(SE_resnet_model.state_dict(), save_model_path)
    ...
    if (epoch + 1) == num_epochs:
        ...
        if (epoch + 1) % save_every == 0:
            save_model_path = os.path.join(save_model_dir, "SE_resnet_e{}_model.pt".format(epoch + 1))
            torch.save(SE_resnet_model.state_dict(), save_model_path)
