...
parser.add_argument('--id', type=str, default='resnet')
...
for epoch in range(num_epochs):
    ...
    test_moderate_acc = test(resnet_model, test_moderate_classes, device)
    ...
    if (epoch + 1) == num_epochs:
        save_model_path = os.path.join(save_model_dir, "basic_resnet_final_model.pt")
        torch.save(resnet_model.state_dict(), save_model_path)
        wandb.save(save_model_path)
    else:
        if (epoch + 1) % save_every == 0:
            save_model_path = os.path.join(save_model_dir, "basic_resnet_e{}_model.pt".format(epoch + 1))
            torch.save(resnet_model.state_dict(), save_model_path)
