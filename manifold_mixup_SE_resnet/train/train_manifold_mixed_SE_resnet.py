...
parser.add_argument('--id', type=str, default='manifold_mixed_and_SE_resnet')
...
for key, img_list in test_img_list.items():  # unsolved!!!!!!!!!!!!!!!!!!!!!
...
manifold_mixed_SE_resnet_model = se_resnet18(9, if_mixup=True, if_se=True, if_shake_shake=False)
...
manifold_mixed_SE_resnet_model = manifold_mixed_SE_resnet_model.to(device)
...
optimizer = optim.Adam(manifold_mixed_SE_resnet_model.parameters(), lr=args.lr)

num_epochs = 1000
for epoch in range(num_epochs):
    manifold_mixed_SE_resnet_model.train()  # Set model to training mode
    running_loss = 0.0
    correct_train = 0
    total_train = 0
    for images, labels in train_loader:
        ...
        #outputs = manifold_mixed_SE_resnet_model(images)  # Forward pass
        lam = mixup_data(alpha=1)
        lam = torch.from_numpy(np.array([lam]).astype('float32')).to(device)
        outputs, reweighted_target = manifold_mixed_SE_resnet_model(images, lam=lam, target=labels)
    ...
    # Validation
    manifold_mixed_SE_resnet_model.eval()  # Set model to evaluation mode
    val_loss = 0.0
    correct_val = 0
    total_val = 0
    with torch.no_grad():
    ...
    test_major_acc = test(manifold_mixed_SE_resnet_model, test_major_classes, device)
    wandb.log({"Test Accuracy (Major Classes)": test_major_acc}, step=epoch)

    # Log test accuracy for moderate classes
    test_moderate_acc = test(manifold_mixed_SE_resnet_model, test_moderate_classes, device)

    wandb.log({"Test Accuracy (Moderate Classes)": test_moderate_acc}, step=epoch)

    # Log test accuracy for rare classes
    test_rare_acc = test(manifold_mixed_SE_resnet_model, test_rare_classes, device)
    ...
    #     save_model_path = os.path.join(save_model_dir, "manifold_mixed_SE_resnet_final_model.pt")
    #     torch.save(manifold_mixed_SE_resnet_model.state_dict(), save_model_path)
    ...
    if (epoch + 1) == num_epochs:
        ...
        if (epoch + 1) % save_every == 0:
            save_model_path = os.path.join(save_model_dir, "manifold_mixed_SE_resnet_e{}_model.pt".format(epoch + 1))
            torch.save(manifold_mixed_SE_resnet_model.state_dict(), save_model_path)
